...
| R2 | Episodic memory type | `engram-storage/src/stores/episodic.rs` | `stores::episodic::tests` + bench `write_throughput` (P1) | ⬜ Phase 2a |
...
| R4 | Confidence decay (lazy) | `engram-core/src/decay.rs` | `decay::tests` (monotonicity) + bench (P7) | ⬜ Phase 1a/2b |
| R5 | Causal-provenance DAG | `engram-storage/src/dag.rs` | `dag::tests` (chains + cycle reject) + bench (P5) | ⬜ Phase 2d |
...
| P7 | < 500 ns/belief decay, 0 background CPU | `benches/decay_eval.rs` | ⬜ Phase 2b |
...
| Q2 | ≥ 10M fuzz iters, zero crashes (×4 targets) | `fuzz/fuzz_targets/*` (nightly) | ⬜ Phase 1+ |
